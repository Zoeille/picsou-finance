 ...
 * this map (see `docs/features/bank-logos.md`). Every other connector — Amundi, crypto
 * exchanges, Trade Republic, Bourse Direct... — is integrated by hand against a single
 * provider, so there is no catalog to ask for a logo and the asset ships with the frontend
 * instead.
 *
 * Keys are the exact `provider` string the backend writes, matched case-insensitively —
 * `AmundiSyncService.PROVIDER` here, accent and all. For crypto exchanges that string is
 * `ExchangeType.name()` (`CryptoExchangeSyncService.resolveAccount`), hence `MERIA` rather
 * than `Meria`. `provider-logos.test.ts` pins those strings so a rename on either side fails
 * loudly instead of silently dropping the logo.
 *
