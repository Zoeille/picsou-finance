...
- Migration `V54__wallet_ethereum_to_evm.sql` converts `wallet_address.chain` and rewrites `account.external_account_id` (`wallet_ethereum_<id>` → `wallet_evm_<id>`) so migrated wallets keep their account, snapshots, and holdings.
...
- `WalletPort` gains a `validateAddress` hook (no-op by default) so a chain that *can* check its address format offline rejects a typo before the wallet row is persisted, as a `400` rather than a failed first sync leaving an unusable row behind.
- Migration `V54` is covered by `V54WalletEthereumToEvmMigrationTest`, which introduces **Testcontainers** to the project — H2 cannot run the PostgreSQL-flavoured migration chain, and the account-id rewrite is the step whose silent failure would cost users their snapshot history and cost basis. See [testing conventions](../conventions/testing.md#testcontainers--only-for-real-postgresql-behaviour).
