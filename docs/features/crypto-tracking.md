...
- **Rethrowing is only safe because the batch loops are guarded**: `SchedulerService.dailySnapshots` (per account), `SchedulerService.refreshPrices` (per member) and `PriceService.backfillHistoricalPrices` (per ticker) each try/catch their loop body. `dailySnapshots` is `@Transactional`, so without its guard one failing price lookup would abort every remaining account **and member** and roll back the snapshots already written in that run; `backfillHistoricalPrices` runs from `PriceBackfillRunner`, an `ApplicationRunner`, where an unguarded throw fails Spring Boot startup outright. Do not remove a guard without also restoring the swallow-everything catch.
