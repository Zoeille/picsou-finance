...
- **CoinGecko failures return no prices — never an exception**: all three fetch methods swallow their errors and return an empty map, by design. Callers treat a missing price as "not valued this cycle", never "not held" (see the prune gotcha above), so a price blip must not fail a wallet sync. The cost is that severity lives **only in the log**, graded by *whose problem it is* rather than by how loud it feels: upstream failures we can only wait out are **WARN** (429 rate-limit, 5xx outage, timeout — with its duration), while a request we built wrong is **ERROR** (400/404, likely a bad `TICKER_TO_ID` coin id), as is any unexpected exception (with stacktrace). Every line names the tickers, and response bodies are truncated to 200 chars so one bad gateway's HTML page can't fill the log. **If you alert on ERROR, you will not be paged for a CoinGecko outage** — that is deliberate (these run on a scheduler, per ticker, so an outage would otherwise emit ERROR continuously for hours); alert on the WARN lines if you want outage visibility. If you change these methods, keep the empty-map contract or the prune logic becomes destructive.
...
- `V54WalletEthereumToEvmMigrationTest` -- Testcontainers/PostgreSQL: the `ETHEREUM` &rarr; `EVM` migration converts the wallet, rewrites `external_account_id` to exactly what `WalletSyncService` recomputes, preserves holdings and their `averageBuyIn` cost basis, leaves other chains and unrelated external ids untouched, renames the default `"ETHEREUM Wallet"` account without touching user labels (V55), and is idempotent when the real migration file is replayed on already-migrated data
