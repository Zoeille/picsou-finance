...
1. **A bundled asset the account points at** (`Account.logoKey`). Only **on-chain wallets** carry one. It wins over everything else because it is the only source a user chose by hand.
...
- `backend/src/main/java/com/picsou/service/AccountService.java` — `update()` keeps a stored key when the request omits it
...
| Picker shown only where a key already exists | An on-chain wallet is the only account with a genuine choice to make; every other account's logo is determined by its connector or its provider name. Gating on the stored key means no new field has to be plumbed through just to answer "is this a wallet?" | A picker on every `CRYPTO` account (would let an exchange account override its own brand mark); an `isWallet` flag added to `AccountResponse` |
...
- **Changing the account type clears the logo.** A key survives only on a `CRYPTO` account (`AccountService.normalizeLogoKey()`). Retyping a wallet and retyping it back does not restore the mark — the next sync won't re-seed it either, since the connector only writes the key at account creation.
...
- `backend/src/test/java/com/picsou/service/AccountServiceTest.java` — `update()` stores the key the picker sent, keeps the stored one when the request omits it, and drops it when the account is retyped away from `CRYPTO`; `create()` ignores a key sent for a non-crypto account.
